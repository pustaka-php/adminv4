            ...
            <tbody>
                <?php if (!empty($authors[$key]) && is_array($authors[$key])): ?>
                    <?php $sl = 1; foreach ($authors[$key] as $pub): ?>
                        <tr>
                            <td><?= $sl++ ?></td>
                            <td><?= esc($author['publisher_name']) ?></td>
                            <td><?= esc($author['author_id']) ?></td>
                            <td><?= esc($author['author_name']) ?></td>
                            <td><?= esc($author['mobile']) ?></td>
                            <td><?= esc($author['email_id']) ?></td>
                            <td><?= $author['publisher_status'] == 1 ? 'Active' : 'Inactive' ?></td>
                            <td class="text-center">
                                <a href="<?= base_url('tppublisher/tpauthorview/'.$author['author_id']); ?>"
                                   class="w-32-px h-32-px bg-primary-light text-primary-600 rounded-circle d-inline-flex align-items-center justify-content-center"
                                   title="View">
                                    <iconify-icon icon="iconamoon:eye-light" width="18"></iconify-icon>
                                </a>

                                <a href="<?= base_url('tppublisher/tpauthoredit/'.$author['author_id']); ?>"
