...
<h6> Mismatch Stock Validation Pending List</h6>
<table class="zero-config table table-hover mt-4">
    ...
    <tbody>
        <?php if (!empty($mismatch_details['validate'])) : ?>
            <?php $i = 1; foreach ($mismatch_details['validate'] as $row): ?>
                <tr>
                   ...
                   <td style="text-align:center;">
                        <form method="post" action="<?= base_url('stock/mismatchupdate') ?>">
                            <input type="hidden" name="book_id" value="<?= esc($row['book_id']) ?>">
                            <input type="hidden" name="action_type" value="validate">
        ...
        <?php else : ?>
            <tr>
                <td colspan="7" class="text-center">No mismatch validate details found.</td>
            </tr>
        <?php endif; ?>
    </tbody>
</table>
<br>
<h6> Mismatch Stock Books List</h6>
<table class="zero-config table table-hover mt-4">
    <thead>
        <tr>
            <th style="width: 2%; text-align:center;">ID</th>
