...
<div class="container py-4" style="max-width:1500px;">
    ...
    <form id="prospectForm" 
          action="<?= base_url('prospectivemanagement/updateProspect/' . $prospect['id']); ?>" 
          method="post">
        <?= csrf_field() ?>

        <!-- BASIC INFO -->
        <div class="card big-card mb-4 shadow-sm">
            <div class="card-header bg-light fw-semibold py-3">Prospect Basic Information</div>
        ...
        <!-- COMMUNICATION -->
        <div class="card big-card mb-4 shadow-sm">
            <div class="card-header bg-light fw-semibold py-3">Communication & Recommended Plan</div>
            ...
            <div class="card-body py-3">
                <div class="row g-4 align-items-start">
                    ...
                    <!-- EMAIL SENT -->
                    <div class="col-md-4">
                        <label class="form-label mb-1">Email Sent</label>
                        ...
                        <div id="emailSentDate" class="mt-2 <?= $prospect['email_sent_flag'] == 1 ? '' : 'd-none'; ?>">
                            <?php 
                                $emailDate = !empty($prospect['email_sent_date']) 
                                              ...
                                              : '';
                            ?>
                            <input type="date" name="email_sent_date" id="email_sent_date" 
                                   class="form-control" value="<?= $emailDate ?>">
                        </div>
                    </div>

                    <!-- INITIAL CALL -->
                    <div class="col-md-4">
                        <label class="form-label mb-1">Initial Call</label>
                        ...
                        <div id="initialCallDate" class="mt-2 <?= $prospect['initial_call_flag'] == 1 ? '' : 'd-none'; ?>">
                            <?php 
                                $callDate = !empty($prospect['initial_call_date']) 
                                            ...
                                            : '';
                            ?>
                            <input type="date" name="initial_call_date" id="initial_call_date" 
                                   class="form-control" value="<?= $callDate ?>">
                        </div>
                    </div>
                    ...
                    <div class="col-md-12">
                        ...
                        <?= esc($latestRemark['remarks']); ?> 
                        (<?= date('d-m-Y h:i A', strtotime($latestRemark['create_date'])); ?>)
                            </textarea>
                        <?php else: ?>
                            <textarea class="form-control" rows="3" readonly>No previous remark found.</textarea>
                        <?php endif; ?>
                    </div>
